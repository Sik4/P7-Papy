...
class LocalPath(FSBase):
    ...
    def pypkgpath(self):
        ...
        directory upwards which still contains an init.py.
        Return None if a pkgpath can not be determined.
        """
        pkgpath = None
        for parent in self.parts(reverse=True):
            if parent.isdir():
                if not parent.join('init.py').exists():
                    break
    ...
    def pyimport(self, modname=None, ensuresyspath=True):
        ...
        the file (taking init.py files into account) will
        ...
        if modname is None:
            ...
            if modfile.endswith(os.path.sep + "init.py"):
                if self.basename != "init.py":
                    modfile = modfile[:-12]
            try:
